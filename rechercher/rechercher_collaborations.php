...
foreach ($liste_article as $elem) {
    ...
    foreach ($array_id_auteurs_test as $id1) {
        ...
        if ($nombre != 0) {
            ...
            if (strtolower($auteur['organisation']) == 'utt' && !in_array($auteur['id'], $array_id_auteurs)) {
...
?>
<h3>Rechercher collaborations de l'auteur UTT : </h3>
    ...
    <div class="form-group"
        ...
        <select name="id_auteur_rc">
            ...
            foreach ($array_id_auteurs as $value) {
                ...
                echo '<option value="' . $auteur['id'] . '">' . $auteur['nom'] . ' ' . $auteur['prenom'] . ' - ' . $auteur['laboratoire'] . '</option>';
            }
            ?>
        </select>
    </div>
    <input class="btn btn-default" type="submit" name="submit_collaboration" value="Rechercher">
</form>
